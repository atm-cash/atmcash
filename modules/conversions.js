// ATM Cash v3.6 - conversions, defaults, config, language and currency helpers
let defaults = {
  market: { rate: 5.05441, date: "", source: "open.er-api.com", rateVersion: "v3.6", updatedAt: 0, rates: { DKK: 1, THB: 5.05441, EUR: 0.134, USD: 0.146, GBP: 0.114 } },
  homeCurrency: "DKK",
  language: "da",
...
function loadData() {
  try {
    ...
    // v3.6: Revolut og Visa bruger valutaomregnerens THB-kurs som standard.
    // Gammel hardcodet Revolut/Visa-kurs nulstilles ved versionsskift.
    const previousRateVersion = loaded.market?.rateVersion || "";
    if (previousRateVersion !== "v3.6") {
      if (loaded.revolut) delete loaded.revolut.manualRate;
      if (loaded.visa) delete loaded.visa.manualRate;
    }
    if (loaded.revolut) {
    ...
    if (loaded.market) {
      ...
      loaded.market.rateVersion = "v3.6";
    }
    return loaded;
  } catch {
