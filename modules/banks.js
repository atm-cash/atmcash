// ATM Cash v3.7 - bank presets and market/manual rate engine
function goNav(page) {
...
function updateRateStatus() {
  ...
  el.textContent = en ? "Currency converter rate · manual override possible" : "Valutaomregnerkurs · manuel overstyring mulig";
  updateConverterStatus();
}
...
function syncManualCardRatesFromInputs() {
  const market = marketThbRate();

  data.revolut.manualRate = optionalManualRate(data.revolut?.manualRate);
  data.revolut.rate = data.revolut.manualRate || market;
  data.revolut.rateSource = data.revolut.manualRate ? "manual" : "market";

  data.visa.manualRate = optionalManualRate(data.visa?.manualRate);
  data.visa.rawRate = data.visa.manualRate || market;
  data.visa.rate = visaEffectiveRate(data.visa.rawRate, getVisaFxMarkupPercent(data.visa));

  const mc = optionalManualRate(data.mastercard?.manualRate || data.mastercard?.rate);
  data.mastercard.manualRate = mc;
  data.mastercard.rate = mc || Number(data.mastercard?.rate || 0);
}
...
async function updateMarketRateIfNeeded() {
  ...
  await fetchCurrencyConverterRates();
  applyMarketRates();
  persist();
  syncInputs();
  syncHomeCurrencyUi();
